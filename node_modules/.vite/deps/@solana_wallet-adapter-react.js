...
} from "./chunk-QPASZBGW.js";
import "./chunk-3MNTPXTM.js";
import "./chunk-GOCGMA7Z.js";
import "./chunk-PWBYASA3.js";
import "./chunk-WGW2LFOE.js";
import {
  require_react
...
async function startScenario(config) {
  ...
  return { close: () => {
    ...
    const handleOpen = async () => {
      ...
      socket.send(await createHelloReq(ecdhKeypair.publicKey, associationKeypair2.privateKey));
      state = {
      ...
      switch (state.__type) {
        ...
        case "hello_req_sent": {
          if (responseBuffer.byteLength === 0) {
            ...
            socket.send(await createHelloReq(ecdhKeypair2.publicKey, associationKeypair.privateKey));
            state = {
              __type: "hello_req_sent",
              associationPublicKey: associationKeypair.publicKey,
  ...
  try {
    ...
    if (lnaPermission.state === "granted") {
      ...
      if (updatedState === "granted") {
        const modal2 = new LocalConnectionModal();
        await new Promise((resolve, reject) => {
          modal2.addEventListener("close", (event) => {
            if (event) {
              reject(new SolanaMobileWalletAdapterError(SolanaMobileWalletAdapterErrorCode.ERROR_ASSOCIATION_CANCELLED, "Wallet connection cancelled by user", { event }));
            }
          });
          modal2.initWithCallback(async () => {
            resolve(true);
          });
          modal2.open();
        });
    ...
    if (e2 instanceof TypeError && (e2.message.includes("loopback-network") || e2.message.includes("local-network-access"))) {
      return;
    }
    if (e2 instanceof SolanaMobileWalletAdapterError) {
      throw e2;
    }
    throw new SolanaMobileWalletAdapterError(SolanaMobileWalletAdapterErrorCode.ERROR_LOOPBACK_ACCESS_BLOCKED, e2 instanceof Error ? e2.message : "Local Network Access permission unknown");
  }
}
...
var WALLET_ASSOCIATION_TIMEOUT = 3e4;
var LocalSolanaMobileWalletAdapterWallet = class {
    ...
    try {
      ...
      const result = await Promise.race([
        checkLocalNetworkAccessPermission().then(async () => {
          ...
          loadingSpinner.open();
          const result2 = await callback(await wallet);
  ...
  #accountsToWalletStandardAccounts = (accounts) => {
    return accounts.map((account) => {
      ...
      return {
        address: esm_default.encode(publicKey),
        publicKey,
        label: account.label,
        icon: account.icon,
  ...
  #signMessage = async (...inputs) => {
    const { authToken, chain } = this.#assertIsAuthorized();
    const addresses = inputs.map(({ account }) => fromUint8Array2(new Uint8Array(account.publicKey)));
  ...
  #performSignIn = async (input) => {
    ...
    try {
      ...
      return {
        account: {
          ...signedInAccount ?? {
            address: esm_default.encode(toUint8Array2(signedInAddress))
          },
      ...
      try {
        ...
        if (parsed && parsed.accounts) {
          const parsedAccounts = parsed.accounts.map((account) => {
            return {
              ...account,
              publicKey: "publicKey" in account ? new Uint8Array(Object.values(account.publicKey)) : esm_default.decode(account.address)
              // Fallback, get publicKey from address
            };
          });
...
var BaseSolanaMobileWalletAdapter = class extends BaseSignInMessageSignerWalletAdapter {
  supportedTransactionVersions = /* @__PURE__ */ new Set(
  ...
  constructor(wallet, config) {
    ...
    this.#accountSelector = async (accounts) => {
      const selectedBase64EncodedAddress = await config.addressSelector.select(accounts.map(({ publicKey }) => fromUint8Array3(new Uint8Array(publicKey))));
      return accounts.find(({ publicKey }) => fromUint8Array3(new Uint8Array(publicKey)) === selectedBase64EncodedAddress) ?? accounts[0];
    };
    this.#wallet = wallet;
  ...
  get publicKey() {
    if (!this.#publicKey && this.#selectedAccount) {
      try {
        ...
        throw new WalletPublicKeyError(e2 instanceof Error && e2?.message || "Unknown error", e2);
      }
    }
